...
function strengthColor(r: number) {
  ...
  if (s === "strong") return r > 0 ? "text-trace-alt" : "text-trace";
  if (s === "moderate") return "text-ink-soft";
  return "text-ink-soft";
}

function barColor(r: number | null) {
  if (r === null) return "bg-grid-fine/40";
  if (r >= 0.3) return "bg-trace-alt";
  if (r <= -0.3) return "bg-trace";
  return "bg-ink-faint";
}
  ...
  return (
    <div className="border border-ink/12 bg-paper p-6 space-y-5">
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex h-9 w-9 items-center justify-center bg-purple-500/10 border border-purple-500/20">
          <FlaskConical className="h-4 w-4 text-purple-400" />
        </div>
        <div>
          <p className="text-sm font-bold text-ink">Korelasyon Analizi</p>
          <p className="text-xs text-ink-soft">Sentiment → Fiyat (lag etkisi)</p>
        </div>
        <span className="ml-auto text-xs text-ink-soft/60">{sample_size} veri noktası</span>
      </div>

      {!sufficient_data ? (
        <p className="text-sm text-ink-soft">{message ?? "Yeterli veri yok."}</p>
      ) : (
        <>
          {/* Ana bulgu */}
            ...
            <div className={cn(
              ...
              strengthOf(best_lag.correlation) === "strong"
                ? best_lag.correlation > 0
                  ? "bg-trace-alt/8 border-trace-alt/20"
                  : "bg-trace/8 border-trace/20"
                : "bg-grid-fine/40 border-ink/12"
            )}>
              <div className="flex items-start gap-3">
                <div className="mt-0.5 shrink-0">
                  {best_lag.correlation > 0.05
                    ? <TrendingUp className="h-5 w-5 text-trace-alt" />
                    : best_lag.correlation < -0.05
                    ? <TrendingDown className="h-5 w-5 text-trace" />
                    : <Minus className="h-5 w-5 text-ink-soft" />}
                </div>
                <div>
                  <p className="text-sm font-bold text-ink leading-snug">{interpretation}</p>
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-ink-soft/50">
                ...
                return (
                  <div key={lag.lag_hours} className="flex items-center gap-3">
                    <span className={cn(
                      ...
                      isBest ? "font-bold text-ink" : "text-ink-soft"
                    )}>
                      {lag.lag_hours}s
                    </span>
                    ...
                    <div className="flex-1 h-5 relative flex items-center">
                      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-px bg-grid-fine/40" />
                      {r !== null && (
                        <>
                          {r >= 0 ? (
                            <div
                          ...
                          {/* Zero marker */}
                          <div className="absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 h-4 w-px bg-grid-fine/40" />
                        </>
                      )}
                    </div>
                    ...
                    <span className={cn(
                      ...
                      r === null ? "text-ink-soft/30"
                        : isBest ? "font-bold " + strengthColor(r)
                        : strengthColor(r)
                    )}>
                      {r !== null ? `${r > 0 ? "+" : ""}${r.toFixed(3)}` : "—"}
                    </span>
                    <span className="w-6 text-right text-[10px] text-ink-soft/40 shrink-0">
            ...
            {/* Eksen açıklaması */}
            <div className="flex justify-between text-[10px] text-ink-soft/40 px-9">
          ...
          {/* Güven notu */}
          <p className="text-[11px] text-ink-soft/50 leading-relaxed border-t border-ink/12 pt-3">
