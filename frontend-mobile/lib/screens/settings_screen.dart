...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (email.isEmpty || pass.isEmpty) { _showMsg('Email ve şifre gerekli.', error: true); return; }

    setState(() => _loading = true);

    if (_isRegisterMode) {
      final name = _nameCtrl.text.trim();
      if (name.isEmpty) { _showMsg('İsim gerekli.', error: true); setState(() => _loading = false); return; }
      ...
      if (ok) {
        ...
        _showMsg('Kayıt başarısız. Email zaten kullanılıyor olabilir.', error: true);
      }
    } else {
      final token = await ApiService.login(email, pass);
      setState(() => _loading = false);
      if (token != null) {
        setState(() { _token = token; _userEmail = email; });
  ...
  void _logout() {
    setState(() { _token = null; _userEmail = null; });
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text('Hesap ve bildirim ayarları', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
            ...
            if (_message != null)
              Container(
                ...
                decoration: BoxDecoration(
                  color: _messageIsError ? AppColors.pastelRed : AppColors.pastelGreen,
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Text(_message!,
                    style: TextStyle(fontSize: 13, color: _messageIsError ? AppColors.danger : AppColors.primary,
                ...
                if (_token != null) ...[
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text(_userEmail ?? '', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                      ])),
                    ]),
                  ),
                  ...
                  Row(children: [
                    Expanded(child: _TabButton(
                      label: 'Giriş Yap', isSelected: !_isRegisterMode,
                      onTap: () => setState(() => _isRegisterMode = false),
                    )),
                    const SizedBox(width: 8),
                    Expanded(child: _TabButton(
                      label: 'Kayıt Ol', isSelected: _isRegisterMode,
                  ...
                  if (_isRegisterMode) ...[
                    _InputField(controller: _nameCtrl, label: 'Ad Soyad', icon: Icons.person_outline),
                    const SizedBox(height: 10),
                  ],
                  _InputField(controller: _emailCtrl, label: 'Email', icon: Icons.email_outlined,
                      keyboardType: TextInputType.emailAddress),
                  const SizedBox(height: 10),
                  _InputField(controller: _passCtrl, label: 'Şifre', icon: Icons.lock_outline, obscure: true),
                  const SizedBox(height: 14),
                  SizedBox(width: double.infinity, child: ElevatedButton(
                    ...
                    child: _loading
                        ...
                        : Text(_isRegisterMode ? 'Hesap Oluştur' : 'Giriş Yap',
...
class _InputField extends StatelessWidget {
  ...
  Widget build(BuildContext context) => TextField(
    ...
    decoration: InputDecoration(
      ...
      filled: true, fillColor: AppColors.bgLight, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
...
class _SectionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(width: 32, height: 32, decoration: BoxDecoration(color: iconColor.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
            alignment: Alignment.center, child: Icon(icon, color: iconColor, size: 17)),
        const SizedBox(width: 10),
